		...
		switch(CPU.STATE){
			...
			case 6:
				...
				case 142:
					...
					break;
				case 143:
					setState(144, "Ciclo de ejecución", "La *MBR* guardará el valor del primer operando en el primer registro del Banco de registros");
					CPU.REGISTERS.R1 = CPU.REGISTERS.MBR;
					CPU.updateRegisters();
					$('.active').removeClass('active');
					$('#reg_mbr, #reg_r1').addClass('active');
					break;
				case 144:
					setState(145, "Decodificación de instrucción", "El *IR* envía el *dirección de memoria* a la *MAR* y a su vez este lo envia al *Bus de Direcciones*.");
					CPU.REGISTERS.MAR = (CPU.REGISTERS.IR >> 4) & 0x0F;
					CPU.updateRegisters();
					$('.active').removeClass('active');
					$('#reg_mar, #reg_ir').addClass('active');
					break;
				case 145: 
					setState(146, "Ciclo de ejecución", "La *unidad de control* le indica a la *memoria* que busque la dirección y coloque el valor en el *bus de datos*");
					$('.active').removeClass('active');
					$('#ram_value_' + CPU.REGISTERS.MAR).addClass('active');
					break;
				case 146:
					setState(147, "Ciclo de ejecución", "La *unidad de control* coloca el valor del *bus de datos* en el *MBR*");
					CPU.REGISTERS.MBR = CPU.RAM[CPU.REGISTERS.MAR];
					CPU.updateRegisters();
					$('.active').removeClass('active');
					$('#reg_mbr').addClass('active');
					break;
				case 147:
					setState(148, "Ciclo de ejecución", "La *MBR* guardará el valor del primer operando en el segundo registro del Banco de registros");
					CPU.REGISTERS.R2 = CPU.REGISTERS.MBR;
					CPU.updateRegisters();
					$('.active').removeClass('active');
